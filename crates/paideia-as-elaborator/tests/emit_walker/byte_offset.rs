//! Phase 7 m1-003: Unit tests for byte_offset_in_text field population
//!
//! Tests verify that the encoder populates byte_offset_in_text before encoding,
//! allowing relocation sites to use the precise instruction byte offset rather than
//! computing it after encoding (which causes off-by-one errors in multi-call bodies).

use paideia_as_elaborator::EmitWalker;
use paideia_as_ir::{Instruction, InstructionSideTable, IrNodeId, Mnemonic, Operand, RegId};
...
fn mixed_instructions_maintain_correct_offsets() {
    ...
    for (id, mnem, offset) in instructions {
        ...
        let operands = match mnem {
            Mnemonic::Call => {
                smallvec::smallvec![Operand::SymbolRef {
                    name: format!("fn{}", id),
                    addend: 0,
                }]
            }
...
/// Test 6: After-offset computation would fail with off-by-one
/// This test demonstrates why we need byte_offset_in_text:
/// If the encoder tries to compute offset AFTER pushing bytes,
/// it would get buf.bytes.len() which is AFTER the push, not before.
#[test]
