        ...
        loop {
            ...
            if self.at(TokenKind::KwLet) || self.at(TokenKind::KwReturn) {
                ...
                if self.at(TokenKind::Semicolon) {
                    ...
                    // before `}` or `finally`, this expression must be a
                    // statement, not the tail. Wrapping it here prevents a
                    // subsequent iteration from overwriting `tail` and leaving
                    // the current expression orphaned in the AST arena, which
                    // cascades to a silent miscompile via emit_walker's
                    // parent-less Store sweep (#1180).
                    if self.at(TokenKind::RBrace) || self.at(TokenKind::KwFinally) {
                        tail = Some(expr);
                        if self.at(TokenKind::KwFinally) {
                            break;
                        }
