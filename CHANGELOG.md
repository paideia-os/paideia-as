...
Closes paideia-as#1352. Third and final primitive of the v0.33 crypto substrate; the other two (Argon2id KDF, ChaCha20-Poly1305 AEAD) landed at #1302/#1305/#1306 in 0.29.x. ML-KEM-768 was the only genuinely new work in v0.33.

- **New crate module `paideia-as-crypto::kem`.** Adds `MlKem768` (three associated functions: `keygen`, `encaps`, `decaps`) and a narrow `KemError` shared with future KEM impls. Byte-buffer surface: 32 B seeds, 1184 B `ek`, 2400 B `dk`, 1088 B `ct`, 32 B `ss` — every size a compile-time constant of the FIPS 203 §7 parameter set. `MlKem768` is `no_std + alloc`-clean (matches every other primitive in the crate). Backend is the RustCrypto [`ml-kem`] crate v0.2 with the `deterministic` feature so seed-driven KeyGen and Encaps reproduce byte-identically against NIST ACVP known-answer vectors. Same "phase-1 FFI over a well-audited Rust crate, phase-6+ paideia-native rewrite behind the same trait" pattern already established for Argon2id and ChaCha20-Poly1305.
- **Three NIST ACVP known-answer vectors embedded and re-exported.** `ACVP_KG_*` (keyGen, tcId 26), `ACVP_EN_*` (encapsulation, tcId 26), `ACVP_DE_*` (decapsulation, tcId 88 "no modification" — the returned `k` is a genuine shared secret rather than an implicit-rejection fallback, so this pins the non-rejection code path). Source: NIST ACVP-Server repository commit `65370b8`, `ML-KEM-{keyGen,encapDecap}-FIPS203/internalProjection.json`. Constants are `pub` so the v0.33-005 integration suite (and any downstream fixture) can pin on them without re-transcribing the ACVP JSON.
- **Three extern-C FFI thunks in `paideia-as-crypto::ffi`.** `paideia_crypto_ml_kem_768_keygen`, `..._encaps`, `..._decaps`. Fixed-size raw-pointer surface (no length arguments, no `written` out-params) because every buffer's length is a compile-time constant of the parameter set — mirrors the `mldsa65_sign_runtime_entry` shape in `paideia-pq-sign`. SysV register mappings documented on each thunk. Null-pointer guards return `PDX_CRYPTO_ERR_INVALID_PARAM`; a `KemError::Primitive` (unreachable on the current backend for fixed-size inputs, kept for future paideia-native impls) maps to `PDX_CRYPTO_ERR_PRIMITIVE`.
- **Elaborator lowering: new `MlKem768` trait dispatcher in `stdlib_lowering::cryptoops`.** `try_lower_ml_kem_768` routes `keygen` / `encaps` / `decaps` through empty SysVRegs recipes whose `extern_target` names the FFI thunks. `mod.rs` adds one arm to the `match trait_name` dispatch. Unknown methods return `None` so a typo (e.g. `MlKem768::keygeen`) falls through to normal call emission and diagnoses T0553 rather than silently emitting a call to an unresolved extern symbol.
- **`.pdx` trait declaration in `crates/paideia-stdlib/pdx/crypto.pdx`.** Adds `trait MlKem768 { fn keygen(...); fn encaps(...); fn decaps(...); }` alongside the existing `Argon2id` and `ChaCha20Poly1305` traits. Same effect row (`!{crypto, mem}`) and capability (`@{paideia.crypto}`) as the other crypto primitives. Header comment documents the implicit-rejection contract explicitly — a `.pdx` caller MUST NOT assume `decaps` errors on tampered ciphertexts; wrap the shared secret in an authenticated transform to detect mismatch.
- **Satellite-runtime re-exports (`paideia-satellite-runtime`).** `pub use` re-export of the three new FFI thunks so satellite `ld -nostdlib` link lines resolve them without changes to individual `build.sh` scripts. Same single-source-of-truth discipline as the existing crypto trio: the `#[unsafe(no_mangle)]` bodies live in `paideia-as-crypto`, cargo's dependency graph forces the object code into the archive, and `ld` finds exactly one definition of each symbol.
- **Consumers.** R108 (paideia-os multi-user identity substrate) will use ML-KEM-768 for device-to-device key agreement; R91+ networking milestones will use it for TLS-1.3-like handshakes. First actual runtime consumption follows once R91's transport layer starts wiring the key-schedule shape.
- **Tests.** Trait level (`paideia_as_crypto::kem::ml_kem_768::tests`): three ACVP vectors + a synthetic-seed round-trip + a determinism-under-fixed-seeds pin + an implicit-rejection pin. FFI level (`paideia_as_crypto::ffi::tests`): the same three ACVP vectors reproduced through the extern-C thunks, plus one NULL-argument guard per thunk. Elaborator level (`paideia_as_elaborator::stdlib_lowering::tests`): each of the three recipes pinned to its FFI symbol name plus an unknown-method fall-through. Runs as `cargo test -p paideia-as-crypto -- ml_kem_768`.
- **Version:** `workspace.version` bumped 0.29.2 → 0.30.0 (minor bump — new public API added: `kem` module, `MlKem768`, three `pub extern "C" fn` thunks, three `stdlib_lowering` recipes, one `.pdx` trait). `find-paideia-as.sh` `MIN_VERSION` unchanged (0.4.0). No git tag this landing per the no-tag-in-session convention; the release tag moves at the next phase close alongside the paideia-os submodule bump.
