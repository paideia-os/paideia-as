...
- **Issue #996** — HashMapU64U64 fixed-capacity open-addressing stdlib (PA-R18-003). Concrete monomorphization `HashMapU64U64` (u64→u64, 32-slot capacity, linear probing, struct-of-arrays layout in .bss). Fibonacci hash function with golden-ratio multiplier 0x9E3779B97F4A7C15 (fold into 5-bit index via shr 59). Operations: `hashmap_u64_new`, `hashmap_u64_put`, `hashmap_u64_get` (returns `OptionU64` via #997 pair ABI), `hashmap_u64_contains`, `hashmap_u64_len`. All function bodies are `unsafe { block: {...} }` (array indexing unsupported in expression position today). Effect row `!{}` on all ops (no rehash path until #996e). Deliverables: canonical stdlib module in `pdx/hashmap_u64_u64.pdx`, documentation surface in `pdx/hashmap.pdx`, parse-only Str-keyed shape anchor in `pdx/hashmap_str_shape.pdx`, 5 build-emit driver fixtures, 5 runtime canaries (new_returns_empty, put_get_roundtrip, get_missing_returns_none, contains_present, collision_probes_sum with 4 keys hashing to slot 0). Deferred: #996b (Str keys, blocked on #1234), #996c (closure values), #996d (remove + tombstone), #996e (heap-backed rehash), #996f (robin-hood), #996g (iter), #996h (bool return pending #1229 bool-return canary).
