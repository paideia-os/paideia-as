...
//! WASM jail) compose individual emit_instruction calls into a complete lowering.
//!
//! The example consumes a WASM function body:
//!   local.get 0 → load arg0 into RAX (evaluation-stack top)
//!   local.get 1 → load arg1 into RBX (evaluation-stack second)
//!   i32.add    → add RAX, RBX
//!   end        → return
//!
//! And emits the corresponding x86_64 bytes, verified round-trip via iced-x86.
//!
//! Run via: cargo run -p paideia-as-emit --example wasm_add
