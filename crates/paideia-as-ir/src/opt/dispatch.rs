...
use super::{
    AlignPass, BranchHintPass, DsePass, InstructionSchedulingPass, MacroFusionPass, NoOpPass,
    OptDiagSink, OptPass, PeepholePass, PoolConstantsPass, TailCallPass, UnrollPass,
};
use crate::IrArena;
...
pub fn canonical_catalog() -> Vec<Box<dyn OptPass>> {
    vec![
        ...
        Box::new(MacroFusionPass),           // m3-007 (O1504)
        Box::new(BranchHintPass),            // m9-007 (O1507)
        Box::new(AlignPass),                 // m3-007 (O1508)
        Box::new(PoolConstantsPass),         // m9-007 (O1509)
        Box::new(TailCallPass),              // m9-008
        Box::new(UnrollPass),                // m9-009
    ]
}
