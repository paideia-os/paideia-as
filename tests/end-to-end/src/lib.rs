//! End-to-end smoke test harness for paideia-as diagnostic codes.
//!
//! See `tests/runner.rs` for the test entry point. The harness walks
//! the `codes/` subdirectory of this crate and asserts:
//!
//! - Each fixture file has a companion `.expect` sidecar listing the
//!   expected diagnostic codes (one code per line, format: `Cxxxx` or `Fxxxx`
//!   or `Sxxxx` or `Txxxx`).
//! - The set of codes emitted by `paideia-as build` on each fixture matches
//!   the expected set (once the walkers are wired and fire on structured IR).
...
/// set of diagnostic codes emitted on stderr (S, F, C, T categories).
///
...
/// Parse a `.expect` sidecar file: one code per line (Cxxxx, Fxxxx, Sxxxx, Txxxx);
