//! `cargo test --test runner -p paideia-opt-align` runs the align corpus.
...
/// Align corpus validation: count fixture files in corpus/
/// to ensure the test harness has the expected 1 fixture.
#[test]
fn align_corpus_has_one_fixture() {
    let dir = corpus_root().join("corpus");
    let files = collect_pdx_files(&dir);
    assert_eq!(
        files.len(),
        1,
        "Expected 1 align corpus fixture, found {}",
        files.len()
    );
}
