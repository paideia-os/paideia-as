...
fn jump_table_dense_fixture_wildcard_ok() {
    ...
    let result = lower_ast_to_ir(&ast, &source_map, &mut sink, &paideia_as_elaborator::StructRegistry::empty(), &paideia_as_elaborator::EnumRegistry::empty(), &std::collections::HashMap::new());
...
fn jump_table_dense_populate_creates_rodata_entries() {
    ...
    // Lower to IR and collect diagnostics.
    let mut sink = VecSink::new();
    let mut result = lower_ast_to_ir(&ast, &source_map, &mut sink, &paideia_as_elaborator::StructRegistry::empty(), &paideia_as_elaborator::EnumRegistry::empty(), &std::collections::HashMap::new());
